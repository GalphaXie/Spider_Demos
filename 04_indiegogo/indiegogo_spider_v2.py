...
class IndiegogoSpider:
    ...
    def get_post_response(self, url, post_data, headers):
        response = requests.post(url, data=post_data, headers=headers)
        assert response.status_code == 200
        return response.content.decode(encoding='utf-8')

    @retry(stop_max_attempt_number=3)
    def get_get_response(self, url, headers):
    ...
    def parse_response(self, resp_str):
        try:
            ...
            raise ValueError("json格式数据解析错误: %s" % e)
        ret_list = []
    ...
    def get_overview_data(self, data_dict):
        try:
            ...
            html = self.get_get_response(url, headers)
    ...
    def get_story_data(self, data_dict):
        ...
        try:
            ...
            resp_json = self.get_get_response(url, headers)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                resp_str = self.get_post_response(self.url, post_data, self.headers)
            except Exception as e:
                print("获取json响应数据失败: %s" % e)
                continue
            else:
            ...
            elif 0 < len(data_list) < 12:  # per_page
                for data_dict in data_list:
                    self.save_data(data_dict)
                break
            else:
