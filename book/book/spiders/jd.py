...
class JdSpider(scrapy.Spider):
    name = 'jd'
    allowed_domains = ['jd.com', 'p.3.cn']  # 这里要配置多个 域名
    ...
    def parse_book_list(self, response):  # 提取列表页的数据
        ...
        for li in li_list:
            ...
            # 发送价格的请求，获取价格 (这里提取的方式是 json 格式的数据)
    ...
    def parse_book_price(self, respose):  # 提取价格
        item = respose.meta["item"]
        item["book_price"] = json.loads(respose.body.decode())[0]["op"]
        print(item)

    # TODO 可以增加爬取商品评论(包括图片等)信息
